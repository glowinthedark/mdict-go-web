# GitHub Actions Workflow for Building and Releasing mdict-server

This directory contains GitHub Actions workflows to build and release the `mdict-server` binary for multiple platforms.

## Workflow: Build and Release

The `build-release.yml` workflow builds the `mdict-server` binary for the following platforms:
- Linux AMD64
- Windows AMD64
- macOS AMD64 (Intel)
...
| Platform | File Name |
|----------|-----------|
| Linux AMD64 | mdict-server-linux-amd64 |
| Windows AMD64 | mdict-server-windows-amd64.exe |
| macOS AMD64 (Intel) | mdict-server-darwin-amd64 |
| macOS ARM64 (Apple Silicon) | mdict-server-darwin-arm64 |
