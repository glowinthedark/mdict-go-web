...
// ---- regexes (RE2 equivalents of the Python patterns) -------------------------
//
// Go's regexp (RE2) has no backreferences or lookaround, so the few Python
// patterns that used \1/\2 backrefs or (?!/) are rewritten to match the same
// inputs structurally (closing quote matched as [ "' ] rather than a backref;
// the abs-ref lookahead handled in code).
...
// mirroring Reader.open() including the FILE.mdd / FILE.1.mdd / FILE.N.mdd scheme.
func openReader(filename string) (*Reader, error) {
	...
	// does not strip them; Python decodes with the dict encoding and .strip()s
	// every key. Normalize here so matching, @@@LINK following, listing and
...
// dictName mirrors the title handling in Reader.open: use the header Title,
...
// string, mirroring the Python `bytes.decode(self._encoding, errors="ignore")`.
...
// treatRecord mirrors MDX._treat_record_data: decode, strip NULs, apply the
// stylesheet when enabled.
...
func (r *Reader) substituteStylesheet(txt string) string {
	...
	for j := 1; j < len(parts); j++ {
		...
		if !ok {
			continue // matches Python: drop the segment when the key is unknown
		}
		p := parts[j]
...
// if none — all accent/case-insensitive (folded) matches. Mirrors MDict.lookup.
...
// lookupPrefix mirrors MDict.lookup_prefix: ALL exact matches, else the first
...
// fold mirrors _fold: casefold + strip combining marks for accent/case-insensitive
// matching.
...
// (exact, cycle-guarded). Mirrors Reader._render.
...
// following @@@LINK. Mirrors Reader.define.
...
// prefix), capped at `limit`. Mirrors Reader.search.
...
// fixDefi mirrors Reader.fixDefi: rewrite internal links to in-dictionary CGI
...
// pyQuote mirrors urllib.parse.quote with the default safe="/": unreserved
...
// via CSS) to ASSET_ROOT, mirroring Reader.extract_assets.
...
// `destNorm` (WAV) via speexdec, once. Mirrors Reader._transcode_spx.
